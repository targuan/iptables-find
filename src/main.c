...
static int for_each_table(int (*func)(const char *, int (*rule_checker)(void*)))
{
    int ret = 1;
    FILE *procfile = NULL;
    ...
    while (fgets(tablename, sizeof(tablename), procfile)) {
        if (tablename[strlen(tablename) - 1] != '\n') {
            perror("Badly formed tablename '%s'\n",tablename);
            exit(1);
        }
        
        tablename[strlen(tablename) - 1] = '\0';
        ret &= func(tablename,&rule_matcher);
    }
...
int main(int argc, char **argv) 
{
    ...
    if(tablename == NULL) {
        ...
        print_table(tablename,&rule_matcher);
    }
    return 0;
}
